...
def majorData():
    folder = "C:/Users/Vivi/Desktop/Statistics/Sta160/AdvisingProject/backend/majorReqs/"

    majors = {
        "Chemistry": folder + "chemistryMajorReqs.csv",
        "Computer Science": folder + "computerscienceMajorReqs.csv",
        "Economics": folder + "economicsMajorReqs.csv",
        "Mathematics": folder + "mathematicsMajorReqs.csv",
        "Physics": folder + "physicsMajorReqs.csv",
        "Statistics": folder + "statisticsMajorReqs.csv"
    }

    majorDf = {}
    for major, filename in majors.items():
        majorDf[major] = pd.read_csv(filename)
    return majorDf
